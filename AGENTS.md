...
| `workflows/quick-flow.yml` | Orchestration: lightweight pipeline for trivial changes (quick plan + approval gate, no spec/tasks) |
...
| `commands/` | Agent system prompts (documentation, documentation-init, finish, quick-plan, quick-implement, quick-review, doc-check) |
...
| `scripts/record-quick-scope.py` | Records the gate-approved Quick Flow `plan.md` hash as the review baseline |
...
model.config.json            ← Optional per-step model overrides (survives reinstall)
specs/                       ← Feature directories, one per feature item
  ...
  <issue>-quick-<slug>/
    instruction.md           ← Written by speckit.extendedflow.quick-plan and quick-implement
    plan.md                  ← Written by speckit.extendedflow.quick-plan (human-gated)
    approved-scope.json       ← Written after quick-plan-gate approval; hashes plan.md
...
- `specs/<issue>-quick-<slug>/` — entire quick directory (instruction, plan, review-findings, doc-check)
...
- **Branch creation and verdict extraction**: See `scripts/create-branch.py`, `scripts/check-converge.py`, `scripts/extract-verdict.py`, `scripts/record-quick-scope.py`, `scripts/preserve-quick-review.py`, `scripts/resolve-bug-context.py`, `scripts/check-bug-verdict.py`
