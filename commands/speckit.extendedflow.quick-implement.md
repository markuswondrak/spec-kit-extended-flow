...
3. **Approved Scope** — read `specs/<feature-dir>/plan.md` and `approved-scope.json`. The latter records the approved plan hash after the gate.
...
3. Read `specs/<feature-dir>/plan.md` and verify `approved-scope.json` identifies it by the current SHA-256 hash before acting on it.
...
- **Follow the approved plan** — implement only the plan's `In Scope` work. Do not implement `Deferred` or `Out of Scope` items. If the plan is wrong, incomplete, missing approval, or its hash no longer matches `approved-scope.json`, report this and stop rather than expanding scope.
