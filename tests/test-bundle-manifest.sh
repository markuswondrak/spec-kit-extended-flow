...
assert_contains "$BUNDLE_FILE" 'id: "spec-kit-unified-flow"' "bundle references unified workflow"
...
UNIFIED_WF_VERSION=$(grep -E '^  version:' "$WORKFLOWS_DIR/unified-flow.yml" | sed -E 's/.*"([^"]+)".*/\1/' | head -n1)
...
assert_contains "$BUNDLE_FILE" "version: \"$UNIFIED_WF_VERSION\"" "bundle references unified workflow version $UNIFIED_WF_VERSION"
...
assert_file_exists "$WORKFLOWS_DIR/unified-flow.yml" "Unified workflow file exists"
